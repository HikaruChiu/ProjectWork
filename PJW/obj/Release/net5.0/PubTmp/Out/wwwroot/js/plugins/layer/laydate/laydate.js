...
; !function (win) {
    ...
    Dates.check = function () {
        ...
        if (value.replace(/\s/g, '') !== '') {
            if (!exp.test(value)) {
                Dates.elem[as.elemv] = '';
                Dates.msg('日期不符合格式，請重新選擇。');
                return 1;
            } else if (isvoid[0]) {
                Dates.elem[as.elemv] = '';
                Dates.msg('日期不在有效期內，請重新選擇。');
                return 1;
            } else {
    ...
    Dates.events = function () {
        ...
        Dates.on(as.otoday, 'click', function () {
            var now = new Date();
            // 2016-09-23 18:20:54 修復選中今天choose方法得不到數據
